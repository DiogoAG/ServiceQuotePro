...
export default function TemplatesPage() {
  ...
  const getItemsForCategory = useCallback((category: string) => {
    if (SERVICE_SUBCATEGORIES[category]) {
      ...
      })).filter(g => g.items.length > 0 || filteredItems.some(i => i.category === `${category} - ${sub}`));
    }
                    ...
                    <AccordionItem key={category} value={category} className={cn(
                      ...
                      <AccordionTrigger className="hover:no-underline py-4 text-left">
                        <div className="flex items-center gap-2">
                          ...
                          {isOffered && <Badge variant="secondary" className="gap-1 text-[8px] bg-primary/10 text-primary border-none"><Star className="w-2.5 h-2.5 fill-primary" /> Offered</Badge>}
                        </div>
                      </AccordionTrigger>
                            ...
                            <div key={group.subName || gIdx} className="space-y-4">
                              {group.subName && (
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <h4 className="text-sm font-bold tracking-tight">{group.subName}</h4>
                                </div>
                              )}
                              
                              <div className="space-y-2">
                                {group.items.map((item) => (
                                  <div key={item.id} className="grid grid-cols-1 md:grid-cols-[1fr_100px_120px_40px] gap-3 md:gap-4 items-start md:items-center p-2 rounded-lg hover:bg-muted/30 transition-colors">
                                    <div className="relative">
                                      <Input 
                                        value={item.description} 
                                        onChange={(e) => handleUpdateCommonItem(item.id, 'description', e.target.value)} 
                                        className={cn("h-9 text-sm border-none focus-visible:ring-1", item.isHardCoded && "opacity-80 cursor-not-allowed")}
                                    ...
                                    <div className="grid grid-cols-2 md:contents gap-2">
                                      <Input value={item.unit || ""} onChange={(e) => handleUpdateCommonItem(item.id, 'unit', e.target.value)} className="h-9 text-sm" readOnly={item.isHardCoded} />
                                      <Input type="number" value={item.defaultUnitPrice} onChange={(e) => handleUpdateCommonItem(item.id, 'defaultUnitPrice', Number(e.target.value))} className="h-9 text-sm" readOnly={item.isHardCoded} />
                                    </div>
                                    {!item.isHardCoded && (
                                      <Button variant="ghost" size="icon" className="text-destructive h-8 w-8" onClick={() => handleRemoveCommonItem(item.id)}>
                ...
                <CardHeader className="p-5 pb-2">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <Badge variant="outline" className="text-[10px] uppercase">{template.serviceCategory}</Badge>
                    </div>
