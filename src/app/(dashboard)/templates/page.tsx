...
import { Plus, Trash2, BookOpen, Copy, Save } from "lucide-react";
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="templates" className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
              ...
              <Card key={template.id} className="relative group">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <Button variant="ghost" size="icon" className="text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleRemoveTemplate(template.id)}>
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </div>
                </CardHeader>
            ...
            <Card className="border-dashed flex items-center justify-center p-12 text-center cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => toast({ title: "Coming Soon", description: "Template creation from here is coming soon. Use 'Save as Template' in the Quote Builder." })}>
